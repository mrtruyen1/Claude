...
> **v7.6:** Audit 2026-06-24 (scheduled full PHASE 0→7) — score **98/100** (MEDIUM CT111 vẫn tồn đọng). **CT111 open-webui VẪN chạy `:main`** (container up 18h, compose đã pin `v0.9.6`) — fix v7.5 CHƯA apply: `pct exec 111 -- sh -c 'cd /opt/open-webui && docker compose down && docker compose up -d'`. **Backup job vmid nay = `101,102,104,106,107,108,110,111,113`** — VM105(n8n) ĐÃ GỠ khỏi job (decommissioned, hợp lý); 108+111 có mặt ✅. VM101 KVM swap **2.45GB ổn định** (2446196 kB, PID 4067694). LVM data% **49.82%**. SMART 3/3 PASSED (UDMA_CRC 18/65/0, temp 38/39/33°C). DSM sata1-4 Reallocated=0. DSM vol 4/56/14%, mdstat UUUU, NCQ 1, net buffer 16MB. 0 OOM, PSI 0.00, snapshots sạch, 0 failed units host+8CT. cloudflared 4 conns/200. vmbr0 drops 1.34M BENIGN. Backup sensor=OK size 3.72GB. CT108 9router next-server v16.2.1 port 20128 (mục đích chưa document). Camera ngoai_troi RTSP timeout sporadic (16:57/17:17 Jun23, watchdog recover). HA stale_restored giảm còn 4 (Glances veth114/dm_20). CT114 vẫn absent ✅.
...
| open-webui image | compose file: `v0.9.6`; **running container: `:main` (v7.6 vẫn BUG, up 18h)** | Fix: `pct exec 111 -- sh -c 'cd /opt/open-webui && docker compose down && docker compose up -d'` |
...
- **(v7.3)** CT108/CT111 mới: **luôn thêm vào backup job khi phát hiện CT mới**. `pvesh set /cluster/backup/<id> --vmid "..."`. Backup job vmid hiện: `101,102,104,105,106,107,108,110,111,113` (VM100 có job daily riêng).
...
- **(v7.5)** **CT108 9router** xác nhận có `next-server (v1)` trên port `20128` (Next.js web app). SSH port 22 cũng mở. Mục đích vẫn chưa document — cần hỏi Truyền. Chưa trong backup-all.sh ctids.
...
**Phiên này READ-ONLY (chưa apply fix — chờ Truyền duyệt):**
- **MEDIUM: CT111 open-webui container VẪN dùng `:main`** (up 18h) dù compose đúng `v0.9.6` — fix v7.5 chưa apply, cần `docker compose down && up -d`. ĐÂY LÀ FINDING DUY NHẤT.
- **CT114 vẫn absent** (resolved) ✅ · **Backup job đã gỡ VM105** (n8n decommissioned) — hợp lý, không phải lỗi.
- Mọi mục khác = baseline hoặc watch đã biết → không trừ điểm thêm.
...
1. **CT111 open-webui `:main`** — MEDIUM. Fix: `pct exec 111 -- sh -c 'cd /opt/open-webui && docker compose down && docker compose up -d'`. Cần Truyền duyệt.
2. **VM101 (HA) KVM swap 2.45GB** — ỔN ĐỊNH (không leo, 2.46→2.45GB). Nếu >3GB → tăng RAM VM101 lên 12GB.
3. **Camera `ngoai_troi` RTSP timeout** — i/o timeout + buffer too small + RTSP wrong input tiếp diễn sporadic (14:17, 14:29, 16:57, 17:17 Jun 23). Watchdog recover, snapshot OK. Monitor tần suất.
4. **LVM data% 49.77%** — ngưỡng action 70%. fstrim tuần tới (Jun 28).
5. **CT108 "9router"** — có Next.js app port 20128, hostname 9router, SSH 22 mở. Mục đích vẫn chưa document + chưa trong backup-all.sh ctids.
6. **backup-all.sh** ctids (102 104 106 107 110 113) chưa có CT108/111 — service data backup gap.
7. **vol3 DSM 14%** (↑ từ 13%) — drift nhỏ, theo dõi.

### Next steps phiên sau
1. **Fix CT111 open-webui**: `pct exec 111 -- sh -c 'cd /opt/open-webui && docker compose down && docker compose up -d'` → verify image = v0.9.6
2. Làm rõ mục đích **CT108 "9router"** (Next.js port 20128); document + thêm vào backup-all.sh nếu có service data
3. Thêm CT108/CT111 vào `backup-all.sh` ctids nếu có service data cần backup
4. Monitor VM101 swap trend (2.45GB)
5. Monitor camera ngoai_troi RTSP timeout frequency (sporadic tăng?)
6. (Tùy) thêm `libata.force=noncq` vào GRUB đủ 3 lớp NCQ
7. **Cập nhật file này** sau phiên tiếp theo
...
|---|---|
| v7.6 | CT111 open-webui `:main` vẫn tồn đọng (MEDIUM −2, fix v7.5 chưa apply); backup job gỡ VM105 (n8n decommissioned, hợp lý); CT108 next-server v16.2.1 port 20128; LVM 49.82%; VM101 swap 2.45GB ổn định; SMART 18/65/0 temp 38/39/33°C; HA stale_restored giảm 959→4; score 98/100 |
