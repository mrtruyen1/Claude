# HOME ASSISTANT SENIOR AUDIT MODE — v8.1

> **Smarthome TruyenND** · cập nhật 2026-06-25
> **v8.1:** Audit 2026-06-25 (full 4-batch, read-only) — HA config **valid**, healthy/supported, **0 config_entry_orphans**, **1 active Repair** `telegram_bot.migrate_chat_ids_in_target_call_service_send_message` (breaks HA 2026.9.0, `chat_id 895298455`, `action_origin: call_service`). Grep YAML xác nhận lại: **KHÔNG có `target:`**, các call `telegram_bot.send_message` đều dùng `chat_id` → repair là runtime/stored, **KHÔNG sửa tay `.storage`**. Baseline khớp: automations.yaml ABSENT · 17 file automation · 16 script · 19 package · 1 command_line · 0 stray ref · DB **278.59 MiB** (statistics 111MB chi phối, states 18MB) · df /config 39%. `stale_restored` **24** Glances transient (gồm veth114 CT114 đã xóa) — BENIGN. **🟡 LOW: Broadlink RM mini t1 `.7` VÀ Mini tang1 2 `.10` Network timeout [-4000]** — `.10` mới so v58 (chỉ có `.7`), nên kiểm cả 2 thiết bị (nguồn/wifi). PVE cross-check (xem PVE v8.1): tất cả fix v8.0 giữ vững, score PVE 97/100. Score HA/PVE **97/100** — còn Telegram repair MEDIUM + Broadlink LOW.
...
| packages | **19** |
| command_line | **1** |
| dead_entities / stale_restored | **0 orphan / 24 stale** (v55 settled baseline; v57 post-restart 06:16 thấy **979** — BENIGN transient khớp v53 pattern: Glances ~300 VIF + mobile_app ~280 + Sonoff cloud ~40 + Frigate ~100 + SystemMonitor ~70 + hassio ~30; tự populate) |
| repairs | **1 active Telegram Bot repair** (v58): `migrate_chat_ids_in_target_call_service_send_message`; YAML đã dùng `chat_id`, issue `action_origin: call_service` |
...
| recorder (v56) | `purge_keep_days:14` · `auto_purge/repack:true` · `commit_interval:60` · DB **~280MB** (bảng `statistics` ~111MB chi phối; `states` ~20MB) — xem §8.2 |
...
### 8.0 · Repairs — Telegram Bot migrate target (v58)
- Active issue: `telegram_bot.migrate_chat_ids_in_target_call_service_send_message`, breaks in HA **2026.9.0**.
...
**Cần xử lý trước HA 2026.9:** Telegram Bot repair `migrate_chat_ids_in_target_call_service_send_message`; chỉ sửa YAML nếu còn `target:`, còn `action_origin: call_service` thì truy nguồn service call/runtime, không xóa `.storage` tay.
