...
| **Frigate CT104** | `192.168.31.104` | `Proxmox:ct_exec ctid=104` | — | — | `nsenter` nếu ct_exec timeout · **LIVE + AI only, KHÔNG lưu trữ** (v7.7): NFS `/volume3/frigate` bỏ (fstab `#DISABLED`); **`record:false` + `snapshots:false`** (no media); chỉ `frigate.db` metadata ở `/opt/frigate/config` (đĩa CT). AI detect chỉ `ngoai_troi`. Bind `/opt/frigate/storage`→host root nay rỗng (harmless) |
...
└── Proxmox Host — PVE 8.x / kernel 6.8.12-30-pve · 192.168.31.84   ← LỚP 1
    ...
    ├── CT 104 — Frigate NVR           192.168.31.104  4096M RAM, swap 2048M, GPU passthrough DRI · 5 cameras · LIVE+AI only (v7.7: KHÔNG lưu trữ — record off + snapshots off; AI detect chỉ ngoai_troi; chỉ frigate.db metadata trên đĩa CT)
...
| Frigate storage (v7.7) | LIVE+AI only, KHÔNG lưu trữ · **`record:false` + `snapshots:false`** (global+per-cam) · chỉ `frigate.db` (18M) ở `/opt/frigate/config` (đĩa CT) · `/opt/frigate/storage`→host root nay rỗng (332K) | clips xuất hiện lại = snapshots bị bật lại; NFS `/volume3/frigate` đã bỏ (fstab `#DISABLED`) |
...
- **(v7.7 — no-storage hoàn chỉnh)** "Không lưu trữ" = TẮT cả `record` LẪN `snapshots`. Chỉ `record:false` chưa đủ — snapshots vẫn ghi ảnh AI (816 file/42M) vào `/opt/frigate/storage` (bind→host root, vì `/media/frigate` không tồn tại). Fix: `snapshots: enabled: false` global + per-camera override (ngoai_troi/phong_ngu cũng có `enabled:true` → phải tắt riêng, vì per-camera override global) + xóa ảnh cũ. `frigate.db` (metadata) ở `/opt/frigate/config` (đĩa CT) là cần thiết cho AI event tracking — KHÔNG xóa. AI detect chỉ bật camera nào có `detect: enabled: true` (hiện chỉ ngoai_troi).
...
1. ✅ **Frigate NFS/storage** — FIXED v7.7 (live+AI only, KHÔNG lưu trữ: record off + snapshots off, đã dọn 816 ảnh cũ). Storage host root nay rỗng (332K). Nếu thấy `/opt/frigate/storage/clips` có file lại → snapshots bị bật lại, kiểm tra config. DSM `/volume3/frigate_new` rỗng — Truyền tự xóa qua DSM UI nếu không dùng.
...
1. **Verify Frigate no-storage giữ vững**: `/opt/frigate/storage/clips` phải rỗng (record off + snapshots off). frigate.db (~18M metadata) trên đĩa CT là bình thường (cần cho AI event tracking).
...
|---|---|
| v7.7 | **FIX APPLY:** 🔴 HIGH Frigate NFS rớt (DSM bỏ share `/volume3/frigate`→`frigate_new` rỗng + mất export → ghi nhầm host root); Truyền xác nhận "live+AI only, KHÔNG lưu trữ" → fstab `#DISABLED` (backup) + **`record:false` + `snapshots:false`** (global+per-cam, backup) + dọn 816 ảnh cũ → storage 332K, AI detect ngoai_troi vẫn chạy (detect_fps 1.8), Frigate healthy. 🟠 MEDIUM failed unit `zigbee2mqtt-reattach` (boot race) → reset-failed + thêm `After/Wants=pve-cluster.service` → 0 failed. Baseline: swap 4.7Gi (VM101 kvm 2.49GB), LVM 50.69%, SMART 18/65/0, DSM vol 4/61/15%, CT 8/8 active, open-webui v0.9.6 healthy. Score 93→100/100 |
