...
- **(v7.7)** `mnt-frigate-nas.mount` failed (timeout/`access denied by server`) = DSM bỏ NFS export `/volume3/frigate`. Frigate chuyển sang **live+AI only, không lưu NAS** (Truyền duyệt) → comment dòng fstab (`#DISABLED-20260624`), `reset-failed`. KHÔNG remount. `/media/frigate` không tồn tại — Frigate ghi `/opt/frigate/storage` (bind→host root), đã thêm `snapshots: retain default 7` chặn phình.
