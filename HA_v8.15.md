# HOME ASSISTANT SENIOR AUDIT MODE — v8.15
> **Smarthome TruyenND** · cập nhật 2026-07-02 (v8.15)
> **v8.15.1 — QUYẾT ĐỊNH TRUYỀN + FIX recorder 2026-07-02 tối (sau audit v8.15, Truyền trả lời 4 câu hỏi):** ① **Fallback Tuya cửa cuốn: KHÔNG SỬA** — Truyền quyết định giữ nguyên, hạ từ MEDIUM xuống **Watch/chấp nhận** (KHÔNG flag lại như MEDIUM ở phiên sau; chỉ báo nếu hành vi thay đổi). ② **Backup CT112 thủ công 15:17 root@pam: CHỦ ĐÍCH** — không flag lại. ③ **SSH host PermitRootLogin/Password yes: GIỮ NGUYÊN** (LAN+tailnet, Truyền chấp nhận). ④ **Recorder exclude CPU Proxmox: DUYỆT + restart.** **🟢 FIX recorder ĐÃ ÁP DỤNG (2 bài học quan trọng):** (a) DB chatty query trả id **`sensor.192_168_31_84_cpu_used`** nhưng đó là **id CŨ đã đổi tên** (row cuối 28/06, tự purge 14d) — entity SỐNG là **`sensor.proxmox_cpu_used`** (5.5k rows, đang tăng). **Bài học: trước khi exclude entity lấy từ DB chatty query, PHẢI verify entity_id còn sống** (`ha_eval_template` search states) — `states_meta` giữ cả id lịch sử sau rename. (b) `sensor.proxmox_cpu_used` nằm sẵn trong **`include.entities`** của `recorder.yaml` (override cũ, lý do nó vẫn ghi DB) → **Bài học: trước khi exclude, PHẢI grep cả khối `include:`** — fix đúng = XÓA dòng khỏi `include.entities` + thêm vào `exclude.entities`. Thực hiện: backup `recorder.yaml.20260702_*.bak`, patch cả 2 khối, `ha core check` OK, **full restart HA Core** (3 lần trong phiên 19:52/20:16/20:20 — 2 lần đầu do exclude nhắm id chết rồi config mơ hồ 2 khối; lần cuối sạch). **VERIFY:** Core RUNNING 2026.7.0, 0 ERROR log, entity sống (15.7%, update bình thường), **DB NGỪNG GHI** (row cuối 13:19:21 UTC < mốc restart 13:20:16 UTC, 6+ phút không row mới), automation 08b vẫn `on` sau 3 restart. Trạng thái cuối phiên: **0 MEDIUM đang chờ** (fallback Tuya đã thành Watch/chấp-nhận theo quyết định Truyền).
> **v8.15 — AUDIT + FIX 2026-07-02 tối (full 4-batch qua MCP, Claude Code):** Score **98/100 sau fix** (0 HIGH, 1 MEDIUM còn lại là fallback Tuya từ v8.14 chờ Truyền duyệt). **🟢 MEDIUM ĐÃ FIX — 08b Telegram alert vỡ vì parse_mode:** cả 3 automation trong `automations/08b_zigbee_theo_doi_thiet_bi.yaml` (mất kết nối / kết nối lại / tín hiệu yếu) nhúng `Entity: {{ trigger.entity_id }}` chứa `_` vào message mà KHÔNG khai `parse_mode` → telegram_bot mặc định **Markdown** → Telegram trả `Can't parse entities: can't find end of the entity` → **×12 lỗi lúc 16:19** khi Z2M update 2.12.1 restart làm thiết bị reconnect hàng loạt — toàn bộ cảnh báo KHÔNG gửi được (`continue_on_error` che lỗi, automation vẫn `on` nhưng tính năng vô hiệu). **FIX:** verify Core 2026.7 hỗ trợ `plain_text` qua service schema → thêm `parse_mode: plain_text` vào cả 3 action (backup `.mcp_backups/08b...bak`), `ha core check` OK, reload automations, 3 automation `on`. **Bài học mở rộng #v8.6:** không chỉ `<>` (HTML) — `_` trong entity_id cũng phá parse Markdown mặc định; MỌI telegram_bot message nhúng `entity_id`/tên kỹ thuật phải khai `parse_mode: plain_text` tường minh (đã quét: `01/02` không gửi entity_id qua telegram, `04` chỉ dùng entity_id trong `{% if %}` — không cần sửa). **🔶 MEDIUM (giữ từ v8.14, chờ duyệt):** fallback Tuya `cover.cong_tac_cua_cuon_thong_minh_curtain` nay `unavailable` (trước `unknown`) — vẫn là fallback trong `scripts/cuacuon_{mo,dong}.yaml`; đề xuất như v8.14 (guard kiểm tra fallback trước khi gọi + cảnh báo "fallback unavailable"), KHÔNG tự sửa vì chạm logic cửa cuốn. **✅ OK:** Core **2026.7.0** (mới nhất), config valid, **0 repairs, 0 orphans** · stale_restored **699** transient BENIGN (mobile_app 261/frigate 148/systemmonitor 79 — pattern quen sau restart Core sáng nay) · **22 file / 40 id automations, 0 dup** · 16 scripts / 20 packages / 1 command_line · `automations.yaml` ABSENT · 0 file rác thư mục load · secrets đủ · `www/` sạch · 0 `target:` telegram · `05` giữ `for:1s`+time · `09e` mutex `from:"off"` · `10_Frigate` `mode:single` · domain: automation **40**, sensor **163**, switch **108** (ổn định 2 phiên — baseline mới, thay 199/110 cũ) · DB `statistics` 110.7MB / `states` 23.1MB, dir 474MB · chatty top `sensor.192_168_31_84_cpu_used` 13.7k (Watch, ứng viên exclude), 2 sensor cover đã exclude rows đang giảm (11.3k/5.7k) · backup sensor `OK` (id 112 = backup CT112 thủ công 15:17 root@pam) · Z2M bridge `on`, SLZB `.45` ping 0% loss, Broadlink `.20` UP, ESPHome `.30` UP · df /config 43%. **Tầng PVE (xem PVE_v8.12): 100/100** — Z2M 2.12.1 update 16:09 (2 fail start transient do build), VM103 RUNNING user-context, swap 4.4Gi PSI 0.00 benign.
> **v8.14.1 — FIX 2026-07-02 trưa:** Đã update **Frigate Server CT104** bằng Docker Compose `stable` từ **0.17.1 → 0.17.2** (`0.17.2-3d4dd3a`). Verify sau update: container `frigate` **healthy**, API `/api/version` OK, `/api/stats` OK, HA `update.frigate_server` đã về **off** (`installed_version=latest_version=0.17.2`), 5/5 camera có `camera_fps/process_fps` ~5fps, Tailscale Serve vẫn **tailnet-only** (không `AllowFunnel`). Lỗi HA `custom_components.frigate` quanh **12:50:40–12:50:55 +07** là transient trong lúc container restart/auth chưa sẵn sàng; log Frigate 90s sau ổn định không còn error/warning. **Đã xử lý tiếp ở v8.14.2:** `shm_size` nâng lên **512MB**, Docker thấy `/dev/dri/renderD128`; VAAPI/QSV không bật vì GPU hiện tại không tương thích an toàn, giữ CPU decode để camera ổn định.
> **v8.14.2 — FIX/VERIFY 2026-07-02 trưa (theo duyệt mục 2/3/5):** Frigate CT104 đã tối ưu phần an toàn: `shm_size` **256MB → 512MB** (stats `/dev/shm total=512MB`, `min_shm=376MB`) và Docker container đã thấy `/dev/dri/renderD128`. Đã test `ffmpeg.hwaccel_args: preset-vaapi` nhưng **rollback ngay** vì FFmpeg crash detect stream, camera về 0fps; nguyên nhân xác nhận ở tầng PVE: `/dev/dri` là **NVIDIA GeForce 210 / nouveau**, không phải Intel iGPU, QSV không có và VAAPI không tương thích với các stream hiện tại. Trạng thái cuối: Frigate **healthy**, 5/5 camera ~5fps, log 60s sạch; giữ detector CPU/no hwaccel để không phá camera. Mục backup job thứ 3 `backup-a70339cc-f258` đã được Truyền xác nhận **OK/chủ đích**, không flag lại.
> **v8.14 — AUDIT 2026-07-02 trưa (full 4-batch qua MCP, Claude Code):** Score **98/100** (0 HIGH, 1 MEDIUM, 0 LOW; còn lại Watch). **🔶 MEDIUM — fallback Tuya cửa cuốn không khả dụng:** `cover.cong_tac_cua_cuon_thong_minh_curtain` hiện `unknown` nhưng vẫn là fallback trong `scripts/cuacuon_mo.yaml` và `scripts/cuacuon_dong.yaml` sau khi Zigbee không phản hồi 3s; action có `continue_on_error:true` nên không làm hỏng script, nhưng thông báo đang nói “đã chuyển sang Tuya Cloud” dù fallback có thể không thực thi được. Không auto-fix vì chạm logic cửa cuốn/an toàn; đề xuất khi Truyền duyệt: kiểm tra fallback entity trước khi gọi Tuya, nếu `unknown/unavailable` thì gửi cảnh báo “fallback Tuya unavailable” thay vì báo đã chuyển. **✅ OK:** Core **2026.7.0**, HAOS **18.1**, config valid, 0 ERROR/WARNING system log, 0 repairs, 0 config_entry_orphans · stale_restored **10** Frigate transient · runtime **40 automations ON**, 22 file automations, 16 scripts, 20 packages, 1 command_line · `automations.yaml` ABSENT · không file rác `.bak/.tmp` trong thư mục load · secrets refs đủ · `www/` chỉ `days.json` + `auto-entities.js` · `05_cau_thang_1` vẫn `for:1s` + time 05:00–22:00 · `06_Sonoff` dùng `value_json.action|default('')` · `10_Frigate` `mode:single`, timeout OK, ảnh qua `/config/.tmp` · `09e` mutex có `from:"off"` · recorder exclude 2 sensor Zigbee chatty active, DB estimate **285MiB**, MariaDB dir **474MB**, `statistics` **189.9MB**, `states` **59.3MB** · backup sensor `OK` **3.76GB** · Z2M bridge `on`, Cầu thang 4 HA LQI **108**, SLZB `.45` ping 0% loss, Broadlink remotes `on`. **👁 WATCH:** `sensor.proxmox_cpu_used`/Proxmox CPU history vẫn chatty (~13.7k rows) nếu không cần history có thể exclude recorder; Frigate Server update **0.17.1→0.17.2** đã xử lý ở v8.14.1; Frigate RTSP `ngoai_troi` timeout cluster 02:46-02:49 đã recover; ESPHome `bedroom` `.29` vẫn `disabled_by:user` nên DOWN benign; VM103 đang ON làm PVE RAM/swap vào watch zone. **Không fix HA áp dụng.**
> **Smarthome TruyenND** · cập nhật 2026-07-02 (v8.13)
> **v8.13 — AUDIT 2026-07-02 chiều (full 4-batch qua MCP, session Claude Code, sau v8.12 cùng ngày):** Score **100/100** (0 HIGH/MEDIUM/LOW, chỉ Watch). **✅ OK toàn bộ:** Core **2026.7.0** RUNNING (VM restart ~2h trước audit) · `ha core check` valid · **0 ERROR + 0 WARNING** trong log · **0 repairs** · 0 config_entry_orphans · automations.yaml ABSENT ✅ · **22 file automations / 40 id, 0 dup** · 16 scripts (format chuẩn, 0 wrapper) · **20 packages** (gồm `claude_routine`) · 1 command_line · secrets đủ 100% (0 MISSING) · `www/` sạch file nhạy cảm · telegram toàn bộ `chat_id` không `target:` ✅ · `10_Frigate` `mode:single` + `wait_for_trigger` có timeout/`continue_on_timeout` ✅ (dòng "mode: restart" chỉ là comment changelog — tránh false-positive grep) · recorder exclude 2 sensor Zigbee chatty (v8.12) đã active, purge 14d/commit 60 ✅ · MariaDB **474MB** (statistics 190MB long-term, states 59MB) · backup sensor `OK` **3.76GB** hôm nay · SLZB `.45` + Broadlink `.20` + ESPHome `.30` ping UP. **👁 WATCH:** stale_restored **561** transient post-restart BENIGN (tiền lệ 959→4 — kiểm lại phiên sau) · sensor domain **163** / switch **108** (thấp hơn baseline 199/110 — drift transient sau restart, đếm lại phiên sau) · **automation domain 40** (baseline cập nhật 35→40, khớp 40 id YAML) · chatty top mới: `sensor.192_168_31_84_cpu_used` ~13.7k rows/14d (Proxmox CPU qua Glances — ứng viên exclude nếu không cần history), 2 sensor cover đã exclude còn rows cũ tự giảm sau purge · **ESPHome `bedroom` `.29` DOWN = BENIGN** — config entry `disabled_by: user` (tắt chủ đích, KHÔNG flag offline) · STRAY hits đều false-positive: comment "KHÔNG qua n8n" trong `15_telegram_lenh_truc_tiep.yaml` + `packages/claude_routine.yaml`, và `misc/recorder.yaml.bak-zigbee-map-20260619` (file backup cũ — cân nhắc dời vào `.mcp_backups/` phiên sau cho đúng §5). **Tầng PVE (xem PVE_v8.10): 100/100** — VM105 n8n ĐÃ XÓA HẲN (VMID 105 = CT zigbee2mqtt-new) + job vzdump thứ 3 sun 04:00 VM100→local đã xác nhận OK/chủ đích ở PVE v8.11.2; VM101 KVM swap chỉ **24MB** (RESOLVED).
> **v8.12 — AUDIT + FIX 2026-07-02 (full 4-batch qua MCP):** Score **98/100**. **🟢 FIXED (LOW):** 9 file rác `.bak`/`.tmp` nằm TRONG thư mục config đang load (`automations/07_...yaml.bak2` + `...bak_dup_fix` + `10_Frigate_Alert_Night.yaml.tmp`; `scripts/cuacuon_{dong,mo}.yaml.bak_*` ×4; `packages/{proxmox_optimize.yaml.bak-20260628, climate_aircon.yaml.bak}`) — HA chỉ load `*.yaml` nên KHÔNG active, nhưng vi phạm §5 (backup phải ở `.mcp_backups/`). Đã `mv` cả 9 file → `/config/.mcp_backups/stray_config_files_20260702_082412/`, thư mục load sạch, `ha core check` OK. **🟡 LOW (baseline drift — đã cập nhật §2):** `automations/` nay **22 file `.yaml`** (baseline cũ ghi 18) — file hợp lệ Truyền thêm dần: `09_telegram_menu`, `11_Nhac_ngay_va_Su_kien`, `13_may_giat_spin_level`, `16_binh_nong_lanh_notify`… ; scripts **16** ✅, packages **20** ✅ (gồm `claude_routine`), command_line **1** ✅; **0 duplicate id automation cấp cao** (grep bắt trigger-id lồng = dương tính giả). **👁 WATCH:** DSM vol2 **65%** (↑, tiến ngưỡng 70) · host swap **2.8GB** nhưng PSI memory **0.00** + **0 OOM** → benign (VM100 KVM 949MB/DSM, glances 331MB, python3 419MB; **VM101 KVM chỉ 24MB** ↓↓ từ 2.60GB baseline — cải thiện lớn) · bảng `statistics` **190MB** (↑ từ 111MB, long-term stats cố ý giữ; DB dir **474MB** <500MB OK). **🟢 FIXED (recorder — Truyền duyệt 2026-07-02):** exclude 2 sensor Zigbee cover chatty khỏi recorder — `sensor.cua_cuon_zigbee_last_seen_2` (~11k rows) + `sensor.cua_cuon_zigbee_lqi_raw` (~5.7k rows, `state_class:measurement` nên còn sinh long-term `statistics`). Cả 2 KHÔNG khớp glob sẵn có do đuôi khác chuẩn (`*_last_seen` vs `..._last_seen_2`; `*_lqi` vs `..._lqi_raw`) → thêm tường minh vào khối `exclude.entities` (cạnh `cua_cuon_zigbee_moving/calibration_time`), backup `recorder.yaml.20260702_091837.bak`, `ha core check` OK, **full restart HA Core** (recorder không reload được) → Core RUNNING lại `2026.7.0`, 2 entity vẫn sống (state live), ngừng ghi DB/statistics từ đây. **✅ OK:** Core **2026.7.0** (mới nhất) · config valid · 0 repairs · 0 config_entry_orphans · stale_restored **561** BENIGN (post-restart 07:13, toàn platform hợp lệ: frigate 148/mobile_app 129/systemmonitor 79/hassio 45/sonoff 42/synology_dsm 33/mqtt 19/hacs 15…) · 05_cửa-cuốn `for:1s`+time 05:00–22:00 ✅ · 09e mutex `from:"off"` ✅ · 10_Frigate `mode:single` ✅ · 06_Sonoff `value_json.action|default('')` ✅ · `allowlist_external_dirs:/config/.tmp` ✅ (active sau restart) · 08 Zigbee `..._state_2` ✅ · Broadlink rm_pro/rm_mini_4c `on` · Proxmox backup **OK 3.76GB** · Zigbee bridge `on` · df /config **42%**.
> **Smarthome TruyenND** · cập nhật 2026-07-01 (v8.11)
> **v8.11 — AUDIT + FIX 2026-07-01 (manual full 4-batch qua MCP):** Score **97/100 sau fix LOW Sonoff**. **🟢 FIXED:** `06_cong_tac_sonoff_cua_cuon.yaml` sinh warning `value_json.action` khi MQTT payload không có field `action` → đã backup `/config/.mcp_backups/06_cong_tac_sonoff_cua_cuon.yaml.bak_valuejson_20260701_121445`, đổi 3 trigger sang `{{ value_json.action | default('') }}`, `ha core check` OK, reload automation OK, `automation.cong_tac_sonoff_dieu_khien_cua_cuon` vẫn `on`. **🔶 MEDIUM (Watch, chưa hành động):** Proxmox host swap **5.2GiB** (VM101 KVM **2.31GiB**, VM100 KVM **0.77GiB**), nhưng PSI memory **0.00**, **0 OOM**, HA ổn → không `swapoff`/restart tự động; hành động nếu VM101 vượt **3GiB** hoặc có PSI/OOM. **🟡 LOW còn lại:** baseline tài liệu lệch: runtime HA `automation=39`, `/config/automations` **21 file**, packages **20** (thêm `claude_routine.yaml`), scripts **16**, command_line **1**; `rest_command.claude_routine_fire` đã load OK và automation `15_telegram_lenh_truc_tiep` active. **🟡 LOW hardening:** cloudflared token nằm trong process argv (đã redact, không ghi secret); cân nhắc chuyển sang credentials-file. **✅ OK:** config valid · 0 repairs · 0 config_entry_orphans · stale_restored **773** BENIGN · `/config/.tmp` allowlist active · `10_Frigate` gửi ảnh qua `/config/.tmp` + `message_thread_id` OK · `/config/www` không còn file nhạy cảm · toàn bộ automation `on` · Frigate Docker healthy, 5 camera, `record:false`, snapshots retain 7d, tailnet-only/no Funnel · Z2M 2.12.0 không lỗi 24h, SLZB ping 0% loss · MariaDB 473MB. **Ghi chú:** HA Core restart từ `core_ssh` lúc **2026-07-01 11:47:53 +07**, RUNNING lại **11:48:46**, verify lại valid.
> **v8.10 — AUDIT 2026-06-30 (scheduled, full 4-batch):** Score **90/100**. **🔴 HIGH (đã patch, CHỜ RESTART):** Automation `10_Frigate_Alert_Night` lỗi lưu/gửi snapshot ảnh cảnh báo đêm **52 lần/ngày** do `configuration.yaml` chưa từng khai báo `homeassistant: allowlist_external_dirs: [/config/.tmp]` → cảnh báo an ninh camera ngoài trời ban đêm KHÔNG gửi kèm ảnh (automation không treo nhờ `continue_on_error`, nhưng tính năng coi như vô hiệu — khoảng trống an ninh thực sự). Đã thêm key vào `configuration.yaml` (backup `.mcp_backups/`), `ha core check` OK; key này thuộc cấp `homeassistant:` nên KHÔNG reload qua API — cần **full restart HA Core** để active. Chưa tự restart (ngoài phạm vi an toàn phiên tự động) — **⚡ CẦN XÁC NHẬN Truyền** để restart. **🔶 MEDIUM (ĐÃ FIX):** `03_Canh_bao_mat_dien.yaml` tái phát thiếu `continue_on_error: true` ở 3 action `notify.telegram_call` (regression sau bản refactor 2026-05-26 gộp UPS bằng `choose`) → đã thêm lại cả 3 chỗ + `ha core check` + reload, verify automation vẫn `on`. **🟡 LOW:** Broadlink `.10` timeout ×2 transient BENIGN (tiền lệ v8.1); MQTT template warning `value_json.action` ×27 trên automation **MỚI** `06_cong_tac_sonoff_cua_cuon.yaml` (Truyền tự tạo 2026-06-30, điều khiển **cửa cuốn** qua nút Sonoff SNZB-01) — chỉ log noise (không ghost-trigger vì giá trị render không khớp payload mong đợi), KHÔNG tự sửa trigger vì chạm logic cửa cuốn, cần Truyền duyệt; DSM vol2 **65%** (↑ nhẹ từ 64%, tiến gần ngưỡng watch 70%). **✅ RESOLVED:** 7 file nhạy cảm trong `www/` (v8.9 LOW ①) đã tự biến mất khỏi `/config/www/` — không còn lộ qua `/local/`. **✅ Baseline:** config valid · 0 repairs · 0 config_entry_orphans · 09e mutex `from:"off"` ✅ · `10_Frigate` `mode:single` ✅ · `05_cau_thang_mo_cua` (cửa cuốn) vẫn `for: 00:00:01` + `condition: time 05:00–22:00` ✅ (fix v8.4.1 còn nguyên) · `telegram_bot.send_message` toàn bộ dùng `chat_id` không `target` ✅ · 0 failed systemd units · SMART 4/4 PASSED · 16 scripts / 19 packages / 1 command_line khớp. **❌ Lệch baseline (cần cập nhật §2 phiên sau):** automations nay **19 file** (thêm `06_cong_tac_sonoff_cua_cuon.yaml` Truyền tự tạo) — đếm lại chính xác id/alias phiên sau. stale_restored 773 (toàn bộ platform hợp lệ đã biết — mobile_app/frigate/systemmonitor/hassio/sonoff/synology_dsm/mqtt/hacs… — BENIGN transient, không xóa). **Báo cáo gửi Telegram OK** qua `telegram_bot.send_message` (chat_id 895298455, plain ASCII tránh lỗi parse `<>`, KHÔNG dùng `notify.telegram_call`/cuộc gọi điện).
...
| automations (dir) | **22 file `.yaml`** (v8.12: +`09_telegram_menu`,`11_Nhac_ngay_va_Su_kien`,`13_may_giat_spin_level`,`16_binh_nong_lanh_notify`; 0 dup id cấp cao) |
...
| automation domain | **40** (tất cả ON; v8.13: khớp 40 id trong 22 file YAML, 0 dup) |
...
- **Full restart bắt buộc cho**: `command_line` switch · patch `.storage/core.config_entries` · sửa entity registry tay · đổi package có entry mới · **đổi `recorder.yaml`** (recorder KHÔNG reload được — exclude mới chỉ live sau restart) · **thêm/sửa key cấp `homeassistant:` (vd `allowlist_external_dirs`)** trong `configuration.yaml` (v8.10 — không có reload API riêng).
...
- **✅ ĐÃ exclude (v8.12, Truyền duyệt 2026-07-02):** `sensor.cua_cuon_zigbee_last_seen_2` (~11k rows) + `sensor.cua_cuon_zigbee_lqi_raw` (~5.7k rows + long-term stats). Bài học: glob `sensor.*_last_seen` / `sensor.*_lqi` **KHÔNG** bắt được entity có hậu tố (`..._last_seen_2`, `..._lqi_raw`) → phải exclude tường minh trong `exclude.entities`. Khi thêm sensor Zigbee diagnostic mới, kiểm tra hậu tố có khớp glob không.
...
**www/ 7 file nhạy cảm (v8.9 LOW) — ✅ RESOLVED (v8.10):** `all_automations_dump.txt`, `frigate_nt_fullview.jpg`, `frigate_nt_snapshot.jpg`, `mcp_run_log.txt`, `ffmpeg_nt.log`, `patch_09_stop.py`, `fix2.py`, `audit_count.py` không còn tồn tại trong `/config/www/` (xác nhận audit v8.10) — không còn lộ qua `/local/` HTTP. Nếu tái xuất hiện, flag lại như LOW.
**Frigate night-alert snapshot lỗi do thiếu `allowlist_external_dirs` (v8.10 — HIGH, đã patch, CHỜ RESTART):** `10_Frigate_Alert_Night.yaml` ghi ảnh vào `/config/.tmp/frigate_alerts/` nhưng `configuration.yaml` chưa khai báo `homeassistant: allowlist_external_dirs: [/config/.tmp]` → HA core chặn ghi file ngoài allowlist mặc định, lỗi 52 lần/ngày, cảnh báo an ninh đêm không kèm ảnh. Đã thêm key (backup `.mcp_backups/`) — **key cấp `homeassistant:` KHÔNG reload qua API, cần full restart HA Core**. Thêm vào danh sách "Full restart bắt buộc cho" ở §8.
...
**Trạng thái:** ✅ v8.10 · HA config valid · **Telegram Bot repair ĐÃ XỬ LÝ** (RESOLVED 2026-06-25) · 0 orphans · 773 stale_restored (toàn bộ platform hợp lệ đã biết, transient) · **19 files** automations (thêm `06_cong_tac_sonoff_cua_cuon.yaml`, đếm lại id/alias phiên sau) · 16 scripts / 19 packages / 1 command_line · DIR-ONLY automation · recorder DB ~348 MiB · backup REST native (dedup) · **Z2M: LXC 105 "zigbee2mqtt-new"** (Z-Stack fw `20260310`) · **Zigbee mesh khỏe — 10/10 online** · Frigate live+AI only, record off + snapshots on retain 7d, **Tailscale tailnet-only** · CT112 Wireguard VPN CHỦ ĐÍCH ✅ · Broadlink .10 timeout ×2 transient (BENIGN) · **✅ www/ 7 file nhạy cảm RESOLVED** (đã tự dọn) · **🔴 CẦN XÁC NHẬN:** restart HA Core để kích hoạt fix `allowlist_external_dirs` (Frigate night-alert snapshot, đã patch config) · **Watch:** VM101 KVM swap **2.34 GB** (↓ nhẹ từ 2.60GB, MEDIUM nhẹ) · DSM vol2 **65%** (↑, tiến tới 70%) · LVM 52.34% · journal 552MB (OK) · MQTT template warning trên `06_cong_tac_sonoff_cua_cuon.yaml` (cần Truyền duyệt fix, chạm cửa cuốn) · ESPHome `.29` ir-smart-hub không ping được (entity vẫn sống) · **SLZB: nên đặt IP tĩnh .45 + update core/ESP fw + cấp nguồn PoE**.
