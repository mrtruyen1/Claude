...
| **openclaw CT114** | dhcp | `Proxmox:ct_exec ctid=114` (khi chạy) | — | — | STOPPED · unprivileged · 4G/20G · onboot=0 · mục đích chưa rõ (v7.4) |
...
└── Proxmox Host — PVE 8.x / kernel 6.8.12-30-pve · 192.168.31.84   ← LỚP 1
    ...
    └── CT 114 — openclaw              STOPPED · unprivileged · 4096M RAM / 20G disk / swap 512 · onboot=0 · vmbr0 dhcp · mục đích chưa rõ (phát hiện v7.4) · CHƯA trong backup job
```
...
| VM101 KVM swap | **2.46GB** (2026-06-23 v7.4: ỔN ĐỊNH, không leo tiếp) | Tăng tiếp >3GB → xem xét tăng RAM VM101 |
...
- **(v7.4)** **CT114 "openclaw" mới phát hiện** (stopped, onboot=0, unprivileged, 4G/20G). CHƯA trong backup job. Khi xác nhận mục đích + có service data → thêm 114 vào job: `pvesh set /cluster/backup/<id> --vmid "101,102,104,105,106,107,108,110,111,113,114"`. Stopped + onboot=0 ⇒ chưa cấp bách, nhưng cần làm rõ mục đích như CT108.
...
- Phát hiện **CT114 "openclaw"** (MEDIUM −2): stopped, onboot=0, unprivileged, 4G/20G, vmbr0 dhcp. CHƯA có trong kiến trúc (đã bổ sung) + CHƯA trong backup job. Mục đích chưa rõ.
- Mọi mục khác = baseline hoặc watch đã biết → không trừ điểm.

**Snapshot baseline xác nhận (2026-06-23 ~22:19):**
Host RAM avail 11Gi (VM103+105 stopped) · swap 4.2Gi (VM101 KVM=2.46GB ỔN ĐỊNH + pvedaemon) · PSI mem avg300=0.00 · 0 OOM · load 2.69/2.23/1.81 · failed units 0 host+0 CT · LVM 55.67% · ZFS ARC 4GiB · SMART 3/3 PASSED UDMA_CRC 18/65/0 · temp 38/40/34°C · NFS 55%/local 58%/lvm 55%. DSM vol 4/56/13% · mdstat UUUU · NCQ sata1-4=1 PASSED. CT tất cả active (114 stopped) · cloudflared 4 conns/200 · MariaDB 470MB · Frigate (healthy 2d) GPU OK · HA 2026.6.4 healthy · 0 repairs · config valid · 0 dead_entities · snapshots sạch · journal 848MB · time synced.

### ⏳ Watch items (monitor phiên sau)
1. **CT114 "openclaw"** — mới, mục đích chưa rõ, chưa trong backup. Làm rõ + thêm backup nếu có data.
2. **VM101 (HA) KVM swap 2.46GB** — ỔN ĐỊNH (không leo từ phiên trước). Nếu >3GB → tăng RAM VM101 lên 12GB.
3. **Camera `ngoai_troi` RTSP timeout** — i/o timeout `.4:554` sporadic. Watchdog recover. Monitor tần suất.
4. **LVM data% 55.67%** — ngưỡng action 70%. fstrim tuần tới.
5. **CT111 open-webui** `.env` `WEBUI_SECRET_KEY` — kiểm tra đã điền chưa (security).
6. **backup-all.sh** ctids chưa có CT108/111/114 — service data backup chưa phủ.
7. **CT108 "9router"** — vẫn chưa rõ mục đích.

### Next steps phiên sau
1. Làm rõ mục đích **CT114 "openclaw"** + **CT108 "9router"**; thêm vào backup job nếu có service data
2. Monitor VM101 swap trend (2.46GB, đang ổn định)
3. Monitor camera ngoai_troi RTSP timeout frequency
4. Kiểm tra `WEBUI_SECRET_KEY` CT111 `.env` đã điền chưa
5. Thêm CT108/111/114 vào `backup-all.sh` ctids nếu có service data cần backup
