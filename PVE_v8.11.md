...
| Backup job vmid | vzdump weekly: **`101,102,104,105,106,107,108,111,112,113`** (v8.7: tự đồng bộ CT105 thay CT110 — PVE-side); `backup-all.sh` ctids: **`(102 104 105 106 107 108 111 113)`** (v8.7 FIX: CT110→CT105, backup `.bak-20260630`); **v8.10:** phát hiện thêm job thứ 3 `backup-a70339cc-f258` sun 04:00 VM100→local keep-last=1 — cần Truyền confirm chủ đích | Thiếu CT mới = backup gap |
...
|---|---|
| v8.11 | **Audit CT102 + nâng cấp MCP server v1.1.0→v1.2.0 (2026-07-02).** CT102 sạch (0 failed, 0 update pending, 0 lỗi journal). FIX: smart_check quét disk động +NVMe (trước bỏ sót nvme0n1); balloon_status/system_overview bỏ hardcode VM list (VM105 stale); NCQ check động; secrets HA_MCP_* chuyển vào .env; dọn 9 file bak + dead code. MỚI: 3 tool audit `backup_status`, `journal_errors`, `security_check` (14→17 tools) — đã verify end-to-end. Baseline mới: NVMe PASSED/Used 11%/Errors 0. |
