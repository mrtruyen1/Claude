# NAS + Proxmox VM Audit & Optimization Prompt — v8.0
...
## 📌 TRẠNG THÁI PHIÊN GẦN NHẤT (2026-06-25 v8.0) — Sau remediation **97/100**

**Phiên v8.0 — Audit + fix đã apply:**
- Score sau sửa **97/100**: Frigate public Funnel HIGH đã đóng, OpenClaw stale unit MEDIUM đã gỡ, Mosquitto owner MEDIUM đã sửa. Còn Telegram Bot repair MEDIUM ở HA và Broadlink timeout LOW.
- Không restart HA/Frigate/Mosquitto; chỉ thay trạng thái Tailscale Serve/Funnel, systemd unit rác và owner file Mosquitto. Backup fix nằm trong `/root/audit-fix-backups/` trên node liên quan.

**Snapshot baseline (2026-06-25 ~08:37 local):**
Host RAM used 16Gi / avail 14Gi · swap 4.0Gi/8.0Gi · load 1.24/1.89/2.02 · **0 failed units** · `openclaw*` units = 0 · LVM thin **50.74%** · SMART 3/3 PASSED UDMA_CRC **18/65/0** stable · NCQ queue_depth=1 · ZFS ARC max 4GiB. DSM `QuangHuy` vol **4%/62%/16%**, mdstat active, net buffer baseline OK. CT 102/104/106/107/108/110/111/113 running · VM100/101 running, VM103 stopped. Frigate Tailscale Serve **tailnet-only** (`frigate.tail1105f.ts.net`, no `AllowFunnel`). Mosquitto config test OK, `passwd`/`acl` = `mosquitto:mosquitto 640`.
