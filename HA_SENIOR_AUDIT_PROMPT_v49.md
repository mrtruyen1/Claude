# HOME ASSISTANT SENIOR AUDIT MODE — v49

> **Smarthome TruyenND** · cập nhật 2026-06-22 (phiên 2)
> v49: xác nhận backup VM100 snapshot OK (DSM hết reboot), fix vzdump CT102 (thêm `/var/lib/nginx /var/cache/apt /var/lib/apt/lists/partial` vào global `/etc/vzdump.conf`), dọn 2 stale_restored entities (`automation.cong_tac_xiaomi_phong_ngu` + `sensor.o_cam_tuya_energy_cost_2`), baseline stale_restored=0, điểm audit 98/100.
...
> 📌 v47→v48: automations **18→17 file**, **33→32 id** sau khi gỡ `06_cong_tac_xiaomi_phong_ngu` (nút Xiaomi `4f68a76f…` mất khỏi Z2M, cần re-pair vật lý mới khôi phục — backup `.mcp_backups/06_cong_tac_xiaomi_phong_ngu.yaml.REMOVED.*.bak`).
