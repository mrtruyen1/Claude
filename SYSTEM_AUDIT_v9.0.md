...
**App Zigbee Map (Android):** kết nối thẳng websocket Z2M (`ws://192.168.31.43:8099`) và inject extension JS qua `bridge/request/extension/save`. Z2M 2.x mặc định `advanced.enable_external_js: false` → app FAIL "Injecting extension into Zigbee2MQTT". Fix (2026-07-03): đặt `enable_external_js: true` trong `configuration.yaml` CT105 (backup `.bak-external-js`), restart z2m, verify roundtrip extension save/remove qua MQTT = ok. Lưu ý bảo mật: cờ này cho phép nạp JS ngoài vào Z2M — chỉ chấp nhận vì Z2M nằm trong LAN, frontend không expose ra ngoài.
