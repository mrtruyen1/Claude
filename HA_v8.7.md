...
> **v8.7 — THÊM 2026-06-29:** Thêm **tile card VM 103 Win10** (`switch.tat_mo_win_10`) vào dashboard T2 (view index 3, card index 4). Card hiển thị màu xanh khi VM đang chạy, xám khi tắt; tap để toggle start/stop. Switch đã tồn tại từ trước trong `command_line/Proxmox.yaml` (`unique_id: tat_mo_win10`, `scan_interval: 600`, `qm start/shutdown 103`); trước đây chỉ hiển thị ở badge home view ("Bật Win10") và dòng trong entities card T2. Tile card mới (`icon: mdi:microsoft-windows`, `color: green`, `tap_action: toggle`) bổ sung hiển thị trực quan hơn. **Không thay đổi YAML command_line, không cần restart HA.**
...
**Tile card VM 103 Win10 (v8.7 — ĐÃ THÊM):** `switch.tat_mo_win_10` đã có tile card tại dashboard T2 (views[3].cards[4]). Icon `mdi:microsoft-windows`, `color: green`, `tap_action: toggle`. Entities card cũ vẫn còn tại views[3].cards[5]. Badge home view ("Bật Win10") vẫn còn tại views[0].badges[5]. Tổng cộng 3 nơi hiển thị switch này.
