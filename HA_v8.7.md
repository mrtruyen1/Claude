# HOME ASSISTANT SENIOR AUDIT MODE — v8.7

> **Smarthome TruyenND** · cập nhật 2026-06-29 (v8.6)
> **v8.7 — THÊM 2026-06-29:** ① **Tile card VM 103 Win10** (`switch.tat_mo_win_10`) thêm vào dashboard T2 (views[3].cards[4]): icon `mdi:microsoft-windows`, `color: green`, `tap_action: toggle` — hiển thị màu xanh khi chạy, xám khi tắt. ② **Automation `15_proxmox_vm103_status_refresh.yaml`** (`id: proxmox_vm103_status_refresh`, `mode: single`, `max_exceeded: silent`) — trigger `time_pattern seconds:/30`, action `homeassistant.update_entity → switch.tat_mo_win_10`; khắc phục vấn đề switch không cập nhật khi VM bật/tắt từ PVE ngoài HA (scan_interval gốc 600s → refresh mỗi 30s qua automation). Không cần restart HA. **Baseline: 18 file / 33 id automations.**
...
| automations (dir) | **18 file / 33 id** (v8.7: +15_proxmox_vm103_status_refresh) |
...
**VM 103 Win10 status button (v8.7 — ĐÃ HOÀN THIỆN):** `switch.tat_mo_win_10` có tile card tại dashboard T2 (views[3].cards[4], icon `mdi:microsoft-windows`, color green). Automation `15_proxmox_vm103_status_refresh` refresh switch mỗi 30s (`homeassistant.update_entity`) → trạng thái cập nhật trong vòng 30s kể cả khi bật/tắt từ PVE trực tiếp. Entities card cũ còn tại views[3].cards[5]; badge home view ("Bật Win10") còn tại views[0].badges[5]. KHÔNG flag automation này là stale — nó chủ ý chạy 2880 lần/ngày.
